...
export default async function ProveedoresPage({
  ...
  const data = await getProveedores(resolvedSearchParams)
      ...
      <Card>
        <CardContent className="pt-6">
          <div className="flex gap-4">
            ...
            <Select defaultValue={resolvedSearchParams.activo || 'all'}>
          ...
          {data.pagination.totalPages > 1 && (
            <div className="flex justify-center gap-2">
              {Array.from({ length: data.pagination.totalPages }, (_, i) => i + 1).map((page) => (
                <Link
                  key={page}
                  href={`/proveedores?page=${page}${resolvedSearchParams.search ? `&search=${resolvedSearchParams.search}` : ''}${resolvedSearchParams.activo ? `&activo=${resolvedSearchParams.activo}` : ''}`}
                >
                  <Button
